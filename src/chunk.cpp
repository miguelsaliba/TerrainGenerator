...
Chunk::Chunk(int size, int offset_x, int offset_z, Perlin &perlin, float triangle_size)
    : size(size), offset_x(offset_x), offset_z(offset_z), perlin(&perlin), triangle_size(triangle_size)
{
...
void Chunk::generate() {
    ...
    for (size_t i = 0; i < indices.size(); i += 3) {
        glm::vec3 v0(vertex_data[indices[i] * 6], vertex_data[indices[i] * 6 + 1], vertex_data[indices[i] * 6 + 2]);
        glm::vec3 v1(vertex_data[indices[i + 1] * 6], vertex_data[indices[i + 1] * 6 + 1], vertex_data[indices[i + 1] * 6 + 2]);
        glm::vec3 v2(vertex_data[indices[i + 2] * 6], vertex_data[indices[i + 2] * 6 + 1], vertex_data[indices[i + 2] * 6 + 2]);
        glm::vec3 normal = glm::normalize(glm::cross(v1 - v0, v2 - v0));

        for (int j = 0; j < 3; ++j) {
            unsigned int index = indices[i + j];
            vertex_data[index * 6 + 3] += normal.x;
            vertex_data[index * 6 + 4] += normal.y;
            vertex_data[index * 6 + 5] += normal.z;
        }
    }
}
